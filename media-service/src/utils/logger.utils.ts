...
const logger: Logger = winston.createLogger({
  ...
  defaultMeta: { service: 'media-service' }, // Useful to identify which service produced the log
  transports: [
    ...
    new winston.transports.File({ filename: 'media-service-error.log', level: 'error' }),
    // Log all messages (info, warn, error, etc.) to combined.log
    new winston.transports.File({ filename: 'media-service-combined.log' }),
  ],
});
