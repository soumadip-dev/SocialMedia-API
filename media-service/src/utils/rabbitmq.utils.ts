...
//* Publishes an event to the exchange using a routing key
async function publishEvent(routingKey: string, message: unknown) {
  ...
  // Convert message to Buffer before sending
  const payload = Buffer.from(JSON.stringify(message));

  channel && channel.publish(EXCHANGE_NAME, routingKey, payload);
