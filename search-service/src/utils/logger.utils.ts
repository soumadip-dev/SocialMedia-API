...
const logger: Logger = winston.createLogger({
  ...
  defaultMeta: { service: 'search-service' }, // Useful to identify which service produced the log
  transports: [
    ...
    new winston.transports.File({ filename: 'search-service-error.log', level: 'error' }),
    // Log all messages (info, warn, error, etc.) to combined.log
    new winston.transports.File({ filename: 'search-service-combined.log' }),
  ],
});
